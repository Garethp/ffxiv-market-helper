...
import { buildMarketPageUrl, fetchCheapestListings } from "./universalis";
...
describe("fetchCheapestListings", () => {
  ...
  describe("for several items", () => {
    it("should ask the region for only the cheapest listing of each item, with no sale history", async () => {
      mockedFetch.mockResolvedValue(json({ items: {} }));

      await fetchCheapestListings("Europe", [8455, 1609]);

      const url = requestedUrl();
      expect(url.pathname).toBe("/api/v2/Europe/8455,1609");
      expect(url.searchParams.get("listings")).toBe("1");
    ...
    it("should give each item's cheapest listing, leaving out items with none", async () => {
      mockedFetch.mockResolvedValue(
        json({
          items: {
            "8455": { listings: [{ pricePerUnit: 100, worldName: "Zodiark" }] },
            "1609": { listings: [] },
          },
        }),
      );

      expect(await fetchCheapestListings("Europe", [8455, 1609, 1])).toEqual(
        new Map([[8455, { pricePerUnit: 100, worldName: "Zodiark" }]]),
      );
    });
    ...
    it("should fail when Universalis doesn't answer successfully", async () => {
      ...
      await expect(
        fetchCheapestListings("Europe", [8455, 1609]),
      ).rejects.toThrow(
        "Universalis cheapest-listings request failed (500) for 2 items in Europe",
      );
    });
  });
  ...
  describe("for a single item", () => {
    it("should ask for the listing fields of a single-item response", async () => {
      ...
      await fetchCheapestListings("Europe", [8455]);
    ...
    it("should give the item's cheapest listing", async () => {
      mockedFetch.mockResolvedValue(
        json({ listings: [{ pricePerUnit: 100, worldName: "Zodiark" }] }),
      );

      expect(await fetchCheapestListings("Europe", [8455])).toEqual(
        new Map([[8455, { pricePerUnit: 100, worldName: "Zodiark" }]]),
      );
    });
    ...
    it("should give nothing for an item with no listings", async () => {
      ...
      expect(await fetchCheapestListings("Europe", [8455])).toEqual(new Map());
    });

    it("should give nothing for an item Universalis doesn't know", async () => {
      mockedFetch.mockResolvedValue(new Response("", { status: 404 }));

      expect(await fetchCheapestListings("Europe", [1])).toEqual(new Map());
    });

    it("should fail when Universalis doesn't answer successfully", async () => {
      mockedFetch.mockResolvedValue(new Response("", { status: 500 }));

      await expect(fetchCheapestListings("Europe", [8455])).rejects.toThrow(
        "Universalis cheapest-listings request failed (500) for 1 items in Europe",
      );
    });
  });
  ...
  it("should ask for North America by the name Universalis gives it", async () => {
    ...
    await fetchCheapestListings("North America", [1, 2]);
  ...
  it("should refuse more items than Universalis takes in one request", async () => {
    ...
    await expect(fetchCheapestListings("Europe", itemIds)).rejects.toThrow(
  ...
  it("should let the request be cancelled", async () => {
    ...
    await fetchCheapestListings("Europe", [1, 2], {
